...
class Robot:
    ...
    def addRobot(self, name='RobotNode'):
        ...
        dummyNode.addObject('MechanicalObject', name="fixedFrame", template="Vec3d", position=[0.028994, 0.029138, 0.168697])
        dummyNode.addObject('RestShapeSpringsForceField', stiffness=1e6, points=[0])
        dummyNode.addObject('UniformMass', totalMass=1)

        # Create constraint between index tip and the dummy mechanical object (3d position)
        constraintBodyIndex = 33
        robotNode = robotWrapperNode.getChild('Robot')
        ...
        constraintsNode.addObject('RigidMapping', template='Rigid,Vec3d', name='robotBodyMapping', index=constraintBodyIndex, input="@Frames/framesDof", output="@attachedToRobotObject")
